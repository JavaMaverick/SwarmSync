...
def plot_comparison(ant_metrics, hybrid_metrics):
    ...
    ant_swarm_cohesion = prepare_data(ant_metrics.get('swarm_cohesion', []), max_frames)
    hybrid_swarm_cohesion = prepare_data(hybrid_metrics.get('swarm_cohesion', []), max_frames)
    ant_target_latency = prepare_data(ant_metrics.get('target_latency', []), max_frames)
    hybrid_target_latency = prepare_data(hybrid_metrics.get('target_latency', []), max_frames)
    ant_fdr = prepare_data(ant_metrics.get('foreign_detection_rate', []), max_frames)
    hybrid_fdr = prepare_data(hybrid_metrics.get('foreign_detection_rate', []), max_frames)
    ant_ptd = prepare_data(ant_metrics.get('pheromone_trail_density', []), max_frames)
    hybrid_ptd = prepare_data(hybrid_metrics.get('pheromone_trail_density', []), max_frames)
    ant_oasr = prepare_data(ant_metrics.get('obstacle_avoidance_success', []), max_frames)
    hybrid_oasr = prepare_data(hybrid_metrics.get('obstacle_avoidance_success', []), max_frames)
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(time_steps, ant_swarm_cohesion, label='Ant-Inspired', color='blue')
    plt.plot(time_steps, hybrid_swarm_cohesion, label='Hybrid Bee-Ant', color='orange')
    plt.xlabel('Time (s)')
    plt.ylabel('Average Distance to Centroid (units)')
    plt.title('Swarm Cohesion Over Time')
    plt.legend()
    plt.grid()
    plt.savefig('swarm_cohesion_comparison.png')
    plt.show()

    # Plot 4: Pheromone Trail Density
    plt.figure(figsize=(10, 6))
    plt.plot(time_steps, ant_ptd, label='Ant-Inspired', color='blue')
    plt.plot(time_steps, hybrid_ptd, label='Hybrid Bee-Ant', color='orange')
    plt.xlabel('Time (s)')
    plt.ylabel('Trails per Target')
    plt.title('Pheromone Trail Density Over Time')
    plt.legend()
    plt.grid()
    plt.savefig('pheromone_trail_density_comparison.png')
    plt.show()
